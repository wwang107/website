      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-6 mt-12">
          {projects.map((p) => (
            <article
              key={p.id}
              className="group bg-[var(--paper-alt)] border border-[var(--paper-border)] rounded-2xl p-6 hover:shadow-lg hover:border-accent-300 transition-all duration-200"
            >
              <div className="flex items-start justify-between gap-4 mb-3">
                <h3 className="text-gray-900 font-bold text-xl group-hover:text-accent-600 transition-colors">
                  {p.title}
                </h3>
              ...
              <div className="flex flex-wrap gap-2">
                {p.tags.map((tag) => (
                  <span key={tag} className="px-2 py-1 bg-accent-50 text-accent-600 text-xs font-medium rounded-md">
                    {tag}
                  </span>
                ))}
              </div>
