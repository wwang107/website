          ...
          {NAV_ITEMS.map((item) => (
            <li key={item}>
              <button
                ...
                className="text-sm text-gray-600 dark:text-gray-300 hover:text-violet-500 dark:hover:text-violet-400 transition-colors font-medium"
              >
                {item}
              </button>
            </li>
        ...
        <div className="md:hidden bg-[#f7f2e8] dark:bg-[#1c1a14] border-t border-[var(--paper-border)] px-6 py-4 flex flex-col gap-4">
          {NAV_ITEMS.map((item) => (
            <button
              key={item}
              onClick={() => scrollTo(item)}
              className="text-left text-gray-700 dark:text-gray-300 hover:text-violet-500 dark:hover:text-violet-400 font-medium"
            >
              {item}
            </button>
          ))}
